...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None,mode=None):
        return checkpoint(self._forward, (x, context,mode), self.parameters(), self.checkpoint)

    def _forward(self, x, context=None,mode=None):
        if self.inout_adp_config and self.adp_enable and mode:
            din = self.inout_adp_config.din
            dout = self.inout_adp_config.dout
            attn1i = x
            
            if din == "attn1i":
                adp = self.adapter_inout[mode](attn1i)
            if dout == "attn1i":
            ...
            if din == "attn1o":
                adp = self.adapter_inout[mode](attn1o)
            attn2i = attn1o + attn1i
            
            if din == "attn2i":
                adp = self.adapter_inout[mode](attn2i)
            
            if dout == "attn2i":
                attn2i = attn2i + adp * self.adp_scale
            if din == "attn2c":
                context = context + self.adp_scale * self.adapter_inout[mode](context)
            attn2o = self.attn2(self.norm2(attn2i), context=context)
            
            if din == "attn2o":
                adp = self.adapter_inout[mode](attn2o)
            ffni = attn2o + attn2i
            
            if din == "ffni":
                adp = self.adapter_inout[mode](ffni)
            ...
            if din == "ffno":
                adp = self.adapter_inout[mode](ffno)
            
            transo = ffno + ffni
            if din == "transo":
                adp = self.adapter_inout[mode](transo)
            if dout == "transo":
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None,mode=None):
        ...
        for block in self.transformer_blocks:
            x = block(x, context=context,mode=mode)
