...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x, emb,mode=None):
        if self.updown:
            ...
            if self.adp_config and self.adp_config.din == "resi" and self.adp_enable and mode:
                adp = self.adapter[mode](x)
            if self.adp_config and self.adp_config.dout == "resi" and self.adp_enable and mode:
        ...
        if self.adp_config and self.adp_config.din == "reso" and  self.adp_config.dout == "reso" and self.adp_enable and mode:
            temp = self.skip_connection(x) + h
            return temp + self.adapter[mode](temp) * self.adp_scale
        elif self.adp_config and self.adp_config.dout == "reso" and self.adp_enable and mode:
...
class UNetModel_inject(nn.Module):
    ...
    def __init__(
        ...
        self.transformer_adp_config = None
        if adapter_config:
            print(adapter_config)
            if hasattr(adapter_config, "resblock_adp_config"):
                self.resblock_adp_config = adapter_config.resblock_adp_config
            if hasattr(adapter_config, "transformer_adp_config"):
                self.transformer_adp_config = adapter_config.transformer_adp_config
            print(self.resblock_adp_config, self.transformer_adp_config, sep="\n")
    ...
    def forward(self, x, timesteps=None, context=None, y=None,mode=None,**kwargs):
        """
        ...
        for module in self.input_blocks:
            h = module(h, emb, context,mode=mode)
            hs.append(h)
        h = self.middle_block(h, emb, context,mode=mode)
