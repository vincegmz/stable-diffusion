...
def create_named_schedule_sampler(name, diffusion_num_timesteps):
    """
    ...
    """
    if name == "uniform":
        return UniformSampler(diffusion_num_timesteps)
    elif name == "loss-second-moment":
        return LossSecondMomentResampler(diffusion_num_timesteps)
    elif name == "lognormal":
...
class UniformSampler(ScheduleSampler):
    def __init__(self, diffusion_num_timesteps):
        self._weights = np.ones([diffusion_num_timesteps])
...
class LossSecondMomentResampler(LossAwareSampler):
    def __init__(self, diffusion_num_timesteps, history_per_term=10, uniform_prob=0.001):
        self.history_per_term = history_per_term
        self.uniform_prob = uniform_prob
        self._loss_history = np.zeros(
            [diffusion_num_timesteps, history_per_term], dtype=np.float64
        )
        self._loss_counts = np.zeros([diffusion_num_timesteps], dtype=np.int)
