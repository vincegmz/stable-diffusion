                ...
                for n in trange(n_iter, desc="Sampling"):
                    for prompts in data:
                        ...
                        for x_sample in x_samples_ddim:
                            ...
                            image_list.append(Image.fromarray(x_sample.astype(np.uint8)))
...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def _train_dataloader(self):
        ...
        print("size of reg set:", len(reg_set))
...
class ImageLogger(Callback):
    ...
    def log_local(self, save_dir, split, images,
        ...
        for k in images:
            ...
            filename = "{}_gs-{:05}_e-{:05}_b-{:04}.jpg".format(
                k,
                global_step,
                current_epoch,
                batch_idx)
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
            ...
            if is_train:
                pl_module.eval()
            if hasattr(pl_module, "class_word") and pl_module.class_word is not None:
                class_word = pl_module.class_word
            ...
            for i in range(len(images)):
                filename = "sample_gs-{:05}_e-{:03}_b-{:03}_{}.jpg".format(
                pl_module.global_step,
                pl_module.current_epoch,
                batch_idx, i)
                images[i].save(os.path.join(path, filename))
...
if __name__ == "__main__":
    ...
    try:
        ...
        model.class_word = opt.class_word
        ...
        default_callbacks_cfg = {
            "setup_callback": {
                "target": "main_dream_booth.SetupCallback",
                "params": {
                    "resume": opt.resume,
                    "now": now,
                    "logdir": logdir,
            ...
            "image_logger": {
                "target": "main_dream_booth.ImageLogger",
                "params": {
                    "batch_frequency": 750,
                    "max_images": 4,
                    "clamp": True
            ...
            "learning_rate_logger": {
                "target": "main_dream_booth.LearningRateMonitor",
                "params": {
            ...
            "cuda_callback": {
                "target": "main_dream_booth.CUDACallback"
            },
        }
        if version.parse(pl.__version__) >= version.parse('1.4.0'):
        ...
        if opt.train:
            try:
                ...
                raise
        end_time = time.time()
        print("total using", end_time - start_time, "s")
        w = DreamWorker(logdir, 0, opt.data_root)
        w.start()
        w.join()
        print(w.res)
        npy_path = os.path.join(logdir, "similar.npy")
        print(f"saving res to {npy_path}")
        np.save(os.path.join(npy_path), w.res)
    except Exception:
